    ...
    return [
      {
        ...
        method: function (job, db,final_cb) {
          let run = async function (json_file, cb) {
            ...
            let rawdata = fs.readFileSync(path.resolve(__dirname, '../../data/' + json_file));
            let json_data = JSON.parse(rawdata);
            console.log(json_data);

            converter.json2csv(json_data.data, (err, csv) => { 
              fs.writeFile(path.resolve(__dirname, '../../data/' + job.id + '.csv'),
                csv,
                function (err) {
